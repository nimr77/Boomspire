...
GameObjectDefinition _def(String id, int version, {List<BuildRequirement> requirements = const []}) {
...
void main() {
  group('needsUpdate', () {
    ...
    test('false when incoming version is equal or lower', () {
      expect(needsUpdate(cached: _def('a', 2), incoming: _def('a', 2)), isFalse);
      expect(needsUpdate(cached: _def('a', 2), incoming: _def('a', 1)), isFalse);
    });
  });
  ...
  group('GameContentSyncService.resolveCatalog', () {
    ...
    test('overlays a cached definition with a higher version than seed', () async {
      final service = GameContentSyncService(
        _FakeRemoteRepo(() => throw StateError('offline')),
    ...
    test('overlays with a newer remote definition and persists it to cache', () async {

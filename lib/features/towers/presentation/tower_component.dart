...
import 'util/paint_tower_overlay.dart';
import 'util/spawn_low_hp_smoke_interval.dart';
import 'util/spawn_tower_idle_phase.dart';
...
abstract class TowerComponent extends PositionComponent
  ...
  double _idlePhase = spawnTowerIdlePhase();
  ...
  void render(Canvas canvas) {
    paintTowerOverlay(
      canvas,
      selected: game.selectedTower.value == this,
      idlePhase: _idlePhase,
      accent: TowerSpriteFactory.accentColor(blueprint.type),
  ...
  void update(double dt) {
    ...
    if (hpRatio < 0.35) {
      ...
      if (_lowHpSmokeTimer <= 0) {
        _lowHpSmokeTimer = spawnLowHpSmokeInterval();
        game.world.spawn(
          SmokeTrailComponent(
